...
| Generative AI API (Claude / GPT-4o / Gemini) | **Gemini** (tries `gemini-2.5-flash` → `gemini-1.5-flash-002` → `gemini-2.0-flash`; retries on 429; override with `GEMINI_MODEL`). |
...
- **Default models:** If `GEMINI_MODEL` is unset, the route tries **`gemini-2.5-flash`**, then **`gemini-1.5-flash-002`**, then **`gemini-2.0-flash`** (404s skip; **429** waits and retries, then tries the next model—free-tier quotas are **per model**). Pin one model with **`GEMINI_MODEL`** if you prefer. See [rate limits](https://ai.google.dev/gemini-api/docs/rate-limits) and [models](https://ai.google.dev/gemini-api/docs/models).
